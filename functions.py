...
def historical(coin, timefrom, timeto):
    ...
    UNIXtimefrom = time.mktime(datetime.datetime.strptime(
        timefrom, "%Y-%m-%d").timetuple())
    UNIXtimeto = time.mktime(datetime.datetime.strptime(
        timeto, "%Y-%m-%d").timetuple())
    ...
    history = cg.get_coin_market_chart_range_by_id(
        id=coin, vs_currency='usd', from_timestamp=UNIXtimefrom, to_timestamp=UNIXtimeto)
    ...
    df = pd.DataFrame(zip(dateList, priceList, market_capList, total_volumesList), columns=[
                      'date', 'usd', 'market_cap', 'volume'])
